...
import { EvalResult, MODEL_CATALOG, ALL_PROMPTS } from "@veritas/shared";
import { generateResponse } from "@veritas/llm-client";
import { runJudge } from "@veritas/evaluator";
import { broadcast } from "./websocket";

// In-memory state for current runs
const activeRuns = new Map<
  string,
  { currentPromptId: string; currentIndex: number; total: number }
>();
...
export function getEvaluationStatus(runId?: string) {
  if (runId && activeRuns.has(runId)) {
    ...
    return {
      ...
      currentPromptIndex: state.currentIndex,
    ...
    for (let i = 0; i < testPrompts.length; i++) {
      ...
      try {
        const [resA, resB] = await Promise.all([
          generateResponse(modelA, promptDef.prompt, []).catch((e) => ({
            content: `ERROR: ${e.message}`,
            latencyMs: 0,
          })),
          generateResponse(modelB, promptDef.prompt, []).catch((e) => ({
            content: `ERROR: ${e.message}`,
            latencyMs: 0,
          })),
        ]);
