  ...
  try {
    ...
    if (inputCheck.triggered) {
      const msgA = {
        ...
        guardrailTriggered: { type: "input", reason: inputCheck.reason },
      };
      const msgB = {
        id: `msg-${Date.now()}-B`,
        role: "assistant",
      ...
      session.messagesA.push({
        id: `usr-${Date.now()}-A`,
        role: "user",
        content: prompt,
      });
      session.messagesB.push({
        id: `usr-${Date.now()}-B`,
        role: "user",
        content: prompt,
      });
    ...
    if (!resA && !resB) {
      return res
        ...
        .json({
          error:
            "Both models are unavailable right now. Please retry in a moment.",
        });
    }
    ...
    const msgB = resB
      ? {
          id: `msg-${Date.now()}-B`,
          role: "assistant",
          content: outputCheckB!.triggered
            ? "Blocked by output guardrail."
            : resB.content,
          latencyMs: resB.latencyMs,
          tokensUsed: resB.usage.promptTokens + resB.usage.completionTokens,
          costUsd: 0,
          guardrailTriggered: outputCheckB!.triggered
            ? { type: "output", reason: outputCheckB!.reason }
            : undefined,
        }
      : {
          id: `msg-${Date.now()}-B`,
    ...
    Promise.all([
      resA
        ? logInference({
            ...
            modelName: modelA.name,
            prompt,
            response: resA.content,
            latencyMs: resA.latencyMs,
            inputTokens: resA.usage.promptTokens,
            outputTokens: resA.usage.completionTokens,
            estimatedCostUsd: 0,
            status: "success",
            guardrailTriggered: outputCheckA!.triggered,
            guardrailReason: outputCheckA!.reason,
          })
        : logInference({
            sessionId: session.id,
            modelId: modelA.id,
      ...
      resB
        ? logInference({
            ...
            modelName: modelB.name,
            prompt,
            response: resB.content,
            latencyMs: resB.latencyMs,
            inputTokens: resB.usage.promptTokens,
            outputTokens: resB.usage.completionTokens,
            estimatedCostUsd: 0,
            status: "success",
            guardrailTriggered: outputCheckB!.triggered,
            guardrailReason: outputCheckB!.reason,
          })
        : logInference({
            sessionId: session.id,
            modelId: modelB.id,
