  ...
  return (
    <div className="max-w-7xl mx-auto p-4 md:p-6 space-y-6 text-left bg-[#0a0a0a] text-[#e0e0e0]">
      ...
      <div className="bg-[#121212] border border-neutral-900 rounded-2xl p-5 shadow-xl flex flex-col lg:flex-row justify-between items-start lg:items-center gap-6">
        <div className="space-y-1">
          ...
          <p className="text-xs text-neutral-400 max-w-xl">
            Live inspect pre-LLM regex filters and post-LLM output decodes. Track execution cost, prompt context safety, refusal quality, and downstream rate limits.
          </p>
        </div>
        ...
        <div className="flex flex-wrap gap-4 items-center w-full lg:w-auto">
          ...
          <button onClick={exportLogsAsJson} disabled={logs.length === 0} className="bg-[#0a0a0a]/80 hover:bg-[#121212]/80 text-neutral-200 border border-neutral-800 px-3.5 py-1.5 rounded-xl text-xs font-semibold flex items-center gap-1.5 transition-all shadow disabled:opacity-40 cursor-pointer">
            <FileJson className="w-4 h-4 text-neutral-500" />Export Audit JSON
          </button>
          <button onClick={fetchLogs} disabled={loading} className="bg-neutral-100 hover:bg-white text-neutral-950 px-4 py-1.5 rounded-xl text-xs font-semibold flex items-center gap-1.5 transition-all shadow disabled:opacity-40 cursor-pointer">
            <RefreshCw className={cn("w-3.5 h-3.5", loading && "animate-spin")} />Sync Logs
          </button>
        </div>
      </div>
      ...
      <div className="grid grid-cols-2 md:grid-cols-4 gap-4">
        <div className="bg-[#121212]/40 border border-neutral-900 rounded-xl p-4 text-left font-mono">
          <span className="text-[9px] uppercase tracking-wider text-neutral-500 block mb-1">Total Logs Evaluated</span>
          <span className="text-lg font-bold text-neutral-100">{totalCalls} calls</span>
        </div>
        <div className="bg-[#121212]/40 border border-neutral-900 rounded-xl p-4 text-left font-mono">
          <span className="text-[9px] uppercase tracking-wider text-neutral-500 block mb-1">Active Guardrail Triggers</span>
          <span className={cn("text-lg font-bold", triggeredAlarms > 0 ? "text-amber-500" : "text-emerald-400")}>{triggeredAlarms} alerts</span>
        </div>
        <div className="bg-[#121212]/40 border border-neutral-900 rounded-xl p-4 text-left font-mono">
          <span className="text-[9px] uppercase tracking-wider text-neutral-500 block mb-1">Avg Latency Speed</span>
          <span className="text-lg font-bold text-neutral-100">{avgLatency} ms</span>
        </div>
        <div className="bg-[#121212]/40 border border-neutral-900 rounded-xl p-4 text-left font-mono">
          <span className="text-[9px] uppercase tracking-wider text-neutral-500 block mb-1">Estimated Operating Costs</span>
          <span className="text-lg font-bold text-emerald-400">${totalCost.toFixed(6)}</span>
        </div>
      </div>
      ...
      <div className="bg-[#121212] border border-neutral-900 rounded-2xl p-4 shadow-xl flex flex-col md:flex-row items-center gap-4">
        <div className="relative flex-1 w-full">
          ...
          <input
            type="text"
            className="w-full bg-[#0a0a0a] border border-neutral-800 rounded-xl py-2 px-10 text-xs text-neutral-300 placeholder-neutral-700 focus:outline-none"
        ...
        <div className="flex items-center gap-1.5 shrink-0 font-mono">
          <button
            ...
            className={cn("px-3 py-1.5 rounded-lg text-2xs transition-all cursor-pointer", filterTriggered === "all" ? "bg-neutral-100 font-bold text-neutral-950 shadow-lg" : "bg-[#0a0a0a] hover:bg-neutral-800 text-neutral-300")}
          >
            All logs
          </button>
          <button
            onClick={() => setFilterTriggered("triggered")}
            className={cn("px-3 py-1.5 rounded-lg text-2xs transition-all flex items-center gap-1 cursor-pointer", filterTriggered === "triggered" ? "bg-amber-600 font-bold text-white shadow-lg" : "bg-[#0a0a0a] hover:bg-neutral-800 text-neutral-300")}
          >
            <ShieldAlert className="w-3.5 h-3.5" /> Guardrail Alarms
          </button>
          <button
            onClick={() => setFilterTriggered("clean")}
            className={cn("px-3 py-1.5 rounded-lg text-2xs transition-all flex items-center gap-1 cursor-pointer", filterTriggered === "clean" ? "bg-emerald-600 font-bold text-white shadow-lg" : "bg-[#0a0a0a] hover:bg-neutral-800 text-neutral-300")}
          >
        ...
        {filteredLogs.length === 0 ? (
          ...
          <div className="overflow-x-auto">
            <table className="w-full text-xs font-mono">
              <thead>
                <tr className="bg-[#0a0a0a] border-b border-neutral-900 text-neutral-400 text-left font-bold select-none">
                    ...
                    <React.Fragment key={log.id}>
                      <tr
                        className={cn(
                          ...
                          log.guardrailTriggered && "bg-amber-950/10 border-l-2 border-l-amber-500"
                        )}
                      ...
                      >
                        <td className="py-3.5 px-4 font-bold">
                          {log.guardrailTriggered ? (
                            <span className="text-amber-500 flex items-center gap-1 animate-pulse">
                              <ShieldAlert className="w-4 h-4 shrink-0" /> TRIGGERED
                            </span>
                          ) : (
                        ...
                        <td className="py-3.5 px-4 text-neutral-300">
                          <span className="flex items-center gap-1"><Clock className="w-3.5 h-3.5 text-neutral-400" /> {log.latencyMs} ms</span>
                        </td>
                        <td className="py-3.5 px-4 text-neutral-300">
                          <span className="flex items-center gap-1"><HardDrive className="w-3.5 h-3.5 text-neutral-500" /> {log.inputTokens + log.outputTokens} t</span>
                        </td>
                        <td className="py-3.5 px-4 text-emerald-400 font-bold">${(log.estimatedCostUsd || 0).toFixed(6)}</td>
                        <td className="py-3.5 px-4 text-neutral-500 text-[9px] font-medium">{new Date(log.timestamp).toLocaleTimeString()}</td>
                        <td className="py-3.5 px-4 font-bold text-neutral-400 hover:text-white shrink-0">{isExpanded ? "Collapse" : "Inspect"}</td>
                      </tr>
                        ...
                        <tr>
                          <td colSpan={7} className="p-4 bg-[#0a0a0a] border-t border-neutral-900">
                            <div className="grid grid-cols-1 md:grid-cols-2 gap-4 text-left font-sans">
                              <div className="space-y-1 bg-[#121212] p-3.5 rounded-lg border border-neutral-900">
                                <span className="text-[9px] uppercase tracking-widest font-mono font-bold text-neutral-400 flex items-center gap-1">
                                  <Terminal className="w-3.5 h-3.5 text-neutral-400" /> Prompt Payload Preview
                                </span>
                              ...
                              <div className="space-y-1 bg-[#121212] p-3.5 rounded-lg border border-neutral-900">
                                <span className="text-[9px] uppercase tracking-widest font-mono font-bold text-neutral-400 flex items-center gap-1">
                                  <Bot className="w-3.5 h-3.5 text-neutral-500 animate-pulse" /> Synthesized Output Payload
                                </span>
                                <p className="text-xs text-neutral-300 whitespace-pre-wrap mt-2 select-all font-mono leading-relaxed max-h-60 overflow-y-auto">
                                  {log.response}
                                </p>
