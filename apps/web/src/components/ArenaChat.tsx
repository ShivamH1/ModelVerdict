...
export default function ArenaChat() {
        ...
        const interval = setInterval(() => {
          ...
          if (!doneA) {
            ...
            setSession(prev => {
              if (!prev) return prev;
              const copy = { ...prev };
              if (copy.messagesA[lastIndexA]) copy.messagesA[lastIndexA] = { ...copy.messagesA[lastIndexA], content: sliceA };
              return copy;
            });
          }
          ...
          if (!doneB) {
            ...
            setSession(prev => {
              if (!prev) return prev;
              const copy = { ...prev };
              if (copy.messagesB[lastIndexB]) copy.messagesB[lastIndexB] = { ...copy.messagesB[lastIndexB], content: sliceB };
              return copy;
            });
          }
        }, 12);
  ...
  const renderBrandLogo = (isModelA: boolean) => {
    ...
    const brand = Object.entries(brands).find(([key]) => targetModelId.includes(key))?.[1] || { bg: "bg-purple-600", char: "Q" };
    return (
  ...
  return (
    <div className="flex flex-col flex-1 h-full bg-[#0a0a0a] text-[#e0e0e0] font-sans antialiased relative">
      <div className="px-4 py-3 bg-[#121212]/30 border-b border-neutral-900 flex justify-between items-center shrink-0">
        <div className="flex items-center gap-3">
          ...
          <button
            ...
            className={cn(
              ...
              blindMode ? "bg-neutral-900 border-neutral-800 text-neutral-300 cursor-pointer" : "bg-neutral-900/10 border-neutral-800/30 text-neutral-500 cursor-pointer"
            )}
          >
        ...
        <div className="flex items-center gap-2">
          <button
            ...
            className={cn("text-xs px-2.5 py-1 rounded transition-all flex items-center gap-1.5", showSettings ? "bg-neutral-800 text-white" : "text-neutral-400 hover:text-white")}
          >
            <Settings className="w-3.5 h-3.5" />
            <span className="hidden sm:inline">Settings</span>
          </button>
          <button onClick={initSession} disabled={loading} className="text-xs text-neutral-300 hover:text-white px-2.5 py-1 rounded transition-all flex items-center gap-1.5 bg-neutral-900 border border-neutral-800 hover:border-neutral-700 disabled:opacity-50">
      ...
      <AnimatePresence>
        {showSettings && (
          <motion.div initial={{ height: 0, opacity: 0 }} animate={{ height: "auto", opacity: 1 }} exit={{ height: 0, opacity: 0 }} className="bg-[#121212] border-b border-neutral-900 overflow-hidden">
            <div className="max-w-4xl mx-auto p-4 space-y-3">
              <div className="text-xs font-semibold text-neutral-200">Configure External OpenAI Endpoint</div>
              <div className="grid grid-cols-1 md:grid-cols-3 gap-3 pt-2">
                <input type="text" className="bg-neutral-950 border border-neutral-800 rounded p-2 text-xs text-neutral-200 focus:outline-none focus:border-neutral-700" placeholder="Base URL" value={customBaseUrl} onChange={(e) => setCustomBaseUrl(e.target.value)} />
                <input type="password" className="bg-neutral-950 border border-neutral-800 rounded p-2 text-xs text-neutral-200 focus:outline-none focus:border-neutral-700" placeholder="Api Key" value={customApiKey} onChange={(e) => setCustomApiKey(e.target.value)} />
                <input type="text" className="bg-neutral-950 border border-neutral-800 rounded p-2 text-xs text-neutral-200 focus:outline-none focus:border-neutral-700" placeholder="Model Name" value={customModelName} onChange={(e) => setCustomModelName(e.target.value)} />
              </div>
            </div>
          </motion.div>
        )}
      ...
      <div className="flex-1 flex flex-col justify-between overflow-y-auto w-full max-w-7xl mx-auto px-4 py-6 md:px-8">
        {!hasMessages ? (
          <ArenaWelcome prompt={prompt} setPrompt={setPrompt} loading={loading} handleSend={handleSend} />
        ) : (
          <BattlePanel 
            session={session!} 
            loading={loading} 
        ...
        {hasMessages && (
          <div className="pt-3 border-t border-neutral-900 w-full max-w-3xl mx-auto select-none">
            <form onSubmit={handleSend} className="flex items-center gap-2 bg-neutral-900/80 rounded-[16px] border border-neutral-800 p-2 focus-within:border-neutral-700 transition-all">
              <input type="text" className="flex-1 bg-transparent text-neutral-200 placeholder-neutral-500 text-xs py-1.5 px-2 focus:outline-none" placeholder={isStreaming ? "Streaming live outputs..." : loading ? "Resolving parallel generations..." : "Prompt both models simultaneously..."} value={prompt} onChange={(e) => setPrompt(e.target.value)} disabled={loading || isStreaming} />
              <button type="submit" disabled={loading || isStreaming || !prompt.trim()} className="w-7 h-7 rounded-full bg-neutral-200 hover:bg-white text-neutral-950 flex items-center justify-center transition-all disabled:opacity-20 cursor-pointer shrink-0">
